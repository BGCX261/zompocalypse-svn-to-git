...
public:
	...
	bool collide(int wall){
		...
		else if((zomb_pt.y+40 >= scrHeight * .77) && wall<=0){
			blocked=false;
			return true;
		}
		else
...
public:
	SuperZombie(float scale, int posx, int posy) : Zombie(scale, posx, posy){Init(scale);}
...
public:
	SpeedZombie(float scale, int posx, int posy) : Zombie(scale, posx, posy){Init(scale);}
...
public:
	RegZombie(float scale, int posx, int posy) : Zombie(scale, posx, posy){Init(scale);}
	void Init(int scale){
		health= 1.5 * scale;
		vel_y=   1.2 * scale;
	}
